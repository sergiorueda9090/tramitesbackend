...
# Host del scraper externo (RUNT / VIN / placa). Migración incremental desde
# el host legacy http://130.94.110.27:8080.
...
def get_user_info_external(request):
    ...
    numero_documento = request.query_params.get('numero_documento', None)
    # El scraper resuelve el nombre por tipo + número de documento
    # (ej. /api/runt/C/1143366949). Por defecto 'C' (Cédula de Ciudadanía),
    # que es el único tipo con el que los demás flujos consultan el titular.
    tipo_documento = request.query_params.get('tipo_documento') or 'C'
    ...
    # Construir URL del servicio RUNT externo
    url = f"{EXTERNAL_SCRAPER_HOST}/api/runt/{tipo_documento}/{numero_documento}"
...
def external_vin(request):
    ...
    url = f"{EXTERNAL_SCRAPER_HOST}/api/vin"
...
def external_runt_vehiculo_vin(request):
    ...
    url = f"{EXTERNAL_SCRAPER_HOST}/api/runt_vehiculo_vin/{vin}"
...
def external_placa(request):
    ...
    del cotizador Cero KM). Consulta el servicio externo de placa, que devuelve
    -entre otros- el campo 'Vin'. El flujo del cotizador toma ese 'Vin' y luego
    consulta el RUNT por VIN (external_runt_vehiculo_vin).
    """
    placa = request.query_params.get('placa') or request.data.get('placa')
    if not placa:
        return Response(
            {"error": "Se requiere el parámetro 'placa'."},
            status=status.HTTP_400_BAD_REQUEST
    ...
    url = "http://130.94.105.156:9514/api/placa"
    payload = json.dumps({"NumberPlate": placa}).encode('utf-8')
    ...
    try:
        ...
        body = e.read().decode('utf-8', errors='replace')
        return Response(
            {"error": f"Error del servicio de placa: {e.code}", "detalle": body},
            status=e.code if 400 <= e.code < 600 else status.HTTP_502_BAD_GATEWAY
        )
    except urllib.error.URLError as e:
        return Response(
            {"error": f"No se pudo conectar al servicio de placa: {str(e.reason)}"},
            status=status.HTTP_502_BAD_GATEWAY
        )
    except Exception as e:
        return Response(
            {"error": f"Error inesperado al consultar la placa: {str(e)}"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
