...
class Tramite(models.Model):
    usuario        = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name='tramites', help_text='Usuario que creó el trámite')
    cliente        = models.ForeignKey(Cliente, on_delete=models.CASCADE, related_name='tramites')
    etiqueta       = models.ForeignKey(Etiqueta, on_delete=models.SET_NULL, null=True, blank=True, related_name='tramites')
    precio_cliente = models.ForeignKey(PrecioCliente, on_delete=models.SET_NULL, null=True, blank=True, related_name='tramites')
    tarifario_soat = models.ForeignKey(TarifarioSoat, on_delete=models.SET_NULL, null=True, blank=True, related_name='tramites', help_text='Tarifa SOAT resuelta')
    ...
    tarifa_codigo       = models.CharField(max_length=100, blank=True, default='', help_text='Snapshot del código de tarifa para historial')
    ...
    # Estados del workflow (mismo patrón que casos_especiales)
    ...
    def __str__(self):
        return f'Trámite {self.id} - {self.placa or "sin placa"} - Cliente: {self.cliente.nombre}'
